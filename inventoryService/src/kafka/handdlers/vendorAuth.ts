...
export const handleVendorAuth = async (token: string): Promise<any> => {
  return new Promise((resolve, reject) => {
    ...
    producer
      .send({
        topic: KAFKA_TOPICS.VENDOR_AUTH_REQUEST_02,
        messages: [{ value: JSON.stringify({ token }) }],
      })
      .then(() => {
        console.log("Message sent successfully for Vendor Auth 02");
      })
      .catch((error) => {
...
export const handleVendorAuthResponse = (response: any) => {
  console.log("Processing VENDOR_AUTH_RESPONSE_02 message:", response);
