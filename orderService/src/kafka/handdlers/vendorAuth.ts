...
export const handleVendorAuth = async (token: string): Promise<any> => {
  return new Promise((resolve, reject) => {
    ...
    producer
      .send({
        topic: KAFKA_TOPICS.VENDOR_AUTH_REQUEST_03,
        messages: [{ value: JSON.stringify({ token }) }],
      })
      .then(() => {
        console.log("Message sent successfully for Vendor Auth 03");
      })
      .catch((error) => {
...
export const handleVendorAuthResponse = (response: any) => {
  console.log("Processing VENDOR_AUTH_RESPONSE_03 message:", response);
  ...
  try {
    ...
    console.error("Error processing 3 message:", error);
  }
};
