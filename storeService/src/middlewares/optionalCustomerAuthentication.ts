...
import { authenticateStoreUser } from "../gRPC/controller/authenticateUser";
...
export const optionalCustomerAuthentication = async (
  ...
  const span: Span = tracer.startSpan("optionalCustomerAuthentication");
  ...
  try {
    ...
    const authResponse: AuthResponse = await context.with(ctx, () => authenticateStoreUser(token, metadata));
